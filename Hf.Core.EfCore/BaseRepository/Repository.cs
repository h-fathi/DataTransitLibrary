...
using Hf.Core.EfCore.GenericRepoitory.Extensions;
...
namespace Hf.Core.EfCore.GenericRepoitory
{
    internal class Repository : IRepository
    {
        ...
        public Task<List<T>> GetListAsync<T>(Filter<T> specification, CancellationToken cancellationToken = default)
           where T : class
        {
        ...
        public async Task<PaginatedList<T>> GetPaginatedListAsync<T>(
            PaginationFilter<T> specification,
            CancellationToken cancellationToken = default)
            where T : class
        {
            if (specification == null)
            ...
            where TProjectedType : class
        {
            ...
            IQueryable<T> query = _dbContext.Set<T>().GetSpecifiedQuery((FilterBase<T>)specification);
        ...
        public Task<T> GetAsync<T>(Filter<T> specification, CancellationToken cancellationToken = default)
            where T : class
        {
            return GetAsync(specification, false, cancellationToken);
        }

        public async Task<T> GetAsync<T>(Filter<T> specification, bool asNoTracking, CancellationToken cancellationToken = default)
            where T : class
        {
            IQueryable<T> query = _dbContext.Set<T>();
        ...
        public async Task AddBulkDataAsync<T>(IEnumerable<T> entities, CancellationToken cancellationToken = default)
              where T : class
        {
            if (entities == null)
            {
            ...
            await _dbContext.BulkInsertAsync(entities.ToList()).ConfigureAwait(false);
